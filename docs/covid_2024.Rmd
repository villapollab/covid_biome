...
![treemu](assets/img/emOTU_itol_tree.png){style="width: 100%"}
...
If you're unsure, check out the MicroViz documentation or read some Amy Willis' [statdivlab](https://github.com/statdivlab) papers!
...
ggsave("final_covid_plots/covid_PHQ_shannon_kruskal.pdf", height = 7, width = 5, units = "in")

```

![shannon](assets/img/covid_PHQ_shannon.pdf)
...
# Save the plot
ggsave("final_covid_plots/covid_PHQ_simpson_kruskal.pdf", height = 7, width = 5, units = "in")

```

![simpson](assets/img/covid_PHQ_simpson.pdf)
...
# Save the plot
ggsave("final_covid_plots/covid_PHQ_chao1_kruskal.pdf", height = 7, width = 5, units = "in")

```

![chao1](assets/img/covid_PHQ_chao1.pdf)
...
ggsave("final_covid_plots/covid_otu_abund_prev.pdf", width = 24, height = 24, units = "in")

# these params were chosen as a compromise with Amy Willis' advice (STAMPS 2024)
```

![otu_abund_prev](assets/img/covid_otu_abund_prev.pdf)
...
ggsave("final_covid_plots/covid_low_bmi_aitch.pdf", height = 5, width = 7, units = "in")
...
![low_bmi_pcoa](assets/img/covid_low_bmi_aitch.pdf)
...
![low_sex_pcoa](assets/img/covid_low_sex_aitch.pdf)
...
![low_age_pcoa](assets/img/covid_low_old_aitch.pdf)
...
```{r, echo=FALSE, code_folding="TRUE", eval=FALSE}
...
ggsave("final_covid_plots/relabund_genus.pdf", height = 5, width = 7, units = "in")
```

![rough_relabund](assets/img/relabund_genus.pdf)
...
![species_severity](assets/img/covid_microbes_sev_htmp_species.pdf)
...
```{r, code_folding="TRUE", eval = FALSE, message=FALSE}
...
```{r, eval=FALSE, code_folding="TRUE", message=FALSE}
...
correlations_df <- vid_mental |>
  tax_model(
    trans = "clr",
    #taxa = tax_top(vid, 30, by = max, rank = "species"),
    rank = "species", 
...
ggsave(filename = "vid_mental_microbe_corr_w_sig.pdf", 
       plot = last_plot(),
       units = "in", 
       width = 12, 
       height = 16, 
...
![spearmans](assets/img/vid_mental_microbe_corr_w_sig.pdf){style="width: 100%"}
...
ggsave("final_covid_plots/covid_mental_rda_phq.pdf", height = 24, width = 28, units = "in")

```

![rda](assets/img/covid_mental_rda_phq.pdf){style="width: 100%"}
...
ggsave(filename = "final_covid_plots/spearman_mental_microbe_net.pdf", units = "in", height = 10, width = 12)

```

![network_rough](assets/img/spearman_mental_microbe_net (1).pdf){style="width: 100%"}
